...
```{r,echo=FALSE,fig.retina = 2, fig.cap ="Education Frequency"}
library(ggplot2)  # For plotting
library(png) 
...
knitr::include_graphics(img_path)
```
```{r,echo=FALSE,fig.retina = 1, fig.cap ="Education distribution"}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 2, fig.cap ="Occupation frequency"}
library(ggplot2)  # For plotting
library(png) 
...
```
```{r,echo=FALSE,fig.retina = 1, fig.cap ="Occupation distribution"}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 2, fig.cap ="Sex feature characteristics"}
library(ggplot2)  # For plotting
library(png) 
...
There are more whites in the labor force in both the Above-50K and at-or-below-50K categories. Blacks and other race follow next. The proportion of Above-50K earners are lesser in other races when compared to whites.


```{r,echo=FALSE,fig.retina = 2, fig.cap ="Marital Status frequency"}
library(ggplot2)  # For plotting
library(png) 
...
knitr::include_graphics(img_path)
```
```{r,echo=FALSE,fig.retina = 1, fig.cap ="Marital Status distribution"}
library(ggplot2)  # For plotting
library(png) 
...
The number of married with civilian spouses are more than any other groups in labor force. The number of Above-50K and at-or-below-50K earners are also more in this group when compared to others. The number of at-or-below-50K are more than the Above-50K in the singles group. This suggests most of this group is below 25 years age group. This can be correlated with the income by age group chart. The number of married with military spouses is less which may be due to lack of data.
...
We can use this to arrive at y_hat_logit. Since we have 7 features \(X = x\) is more like \(X_{i} = x_{i}\) where \(i\) is from 1 to 7.
...
The accuracy for this model is 0.8167 however, sensitivity is 0.4791 and specificity is 0.9268. This indicates this model has higher ratio of negative outcomes than positive outcomes. It does have better accuracy, confidence interval, and F1 score than plain old guessing. Prevalence remains the same. AUC has improved as well.  Lets see if we can do better
...
glm produces accuracy of 0.8098, however, specificity is still higher than sensitivity and prevalence about the same. CI and F1 scores can be better. AUC can also do better. We can do better!

```{r,echo=FALSE,fig.retina = 1, fig.cap ="GLM classification summary"}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 1, fig.cap ="naive bayes classification summary"}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 1, fig.cap ="knn classification summary"}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 1, fig.cap ="knn tune classification summary"}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 1, fig.cap ="rpart classification summary"}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 1, fig.cap ="random forest classification summary"}
library(ggplot2)  # For plotting
library(png) 
...
Adding class has a negative impact on the accuracy and precision when compared to occupation. We can futher investigate into how often a specific feature is used in the predictions using variable importance
...
Further improvements could be made by use random oversampling and/or undersampling techniques to fill the void created by imbalanced datasets.

Another approach for improving accuracy would be to create an essemble based on multiple algorithms like rpart, random forest and glm.
