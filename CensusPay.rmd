...
We will choose 13 attributes, income being be the outcome and the rest 12 attributes being the predictors or features. After perusing the dimensions, we will randomly choose 10% of the dataset to be the test set and remaining 90% will be the training set. 90% is sizable enough to run various algorithms like lm, glm,nb, knn, rpart, and random forest with some tuning.
...
We can also see the number or above-50K income is significantly less than (30%) the at-or-below-50K income. This implies that there is prevalence of at-or-below-50K group in the dataset.
...
```{r,echo=FALSE,fig.retina = 2, fig.cap ="Education statistics" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
Interestingly enough, in comparison to Bachelors and some college, group with masters have lesser Above-50K. Additionally, the group that has at least 12 years of education have more Above-50K than others. There are more at-or-below 50K earners in the high-school grad and less than 10 years of education categories. 

```{r,echo=FALSE,fig.retina = 2, fig.cap ="Years of education" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 2, fig.cap ="Occupation frequency" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
The same groups earn more and are in the Above-50K category.There are fairly significant number of at-or-below-50K earners in the other categories. There are 1600 or so missing data. This makes up 5% of the cleaned dataset.

```{r,echo=FALSE,fig.retina = 2, fig.cap ="Sex feature characteristics" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 2, fig.cap ="Race distribution" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 2, fig.cap ="Marital Status frequency" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
The number of married with civilian spouses are more than any other groups in labor force. The number of Above-50K and at-or-below-50K earners are also more in this group when compared to others. 
...
The number of at-or-below-50K are more than the Above-50K in the singles group. This suggests most of this group is below 25 years age group. This can be correlated with the income by age group chart. The number of married with military spouses is less which may be due to lack of data.


```{r,echo=FALSE,fig.retina = 2, fig.cap ="Relationship distribution" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 2, fig.cap ="Age distribution" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 2, fig.cap ="Hours worked" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,fig.retina = 2, fig.cap ="Class distribution" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="50%", fig.cap ="LM classification summary" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
glm produces accuracy of 0.8105, however, specificity 0.38719 is still higher than sensitivity 0.94864 and prevalence about the same. CI and F1 scores can be better. AUC can also do better. We can do better!

```{r,echo=FALSE,out.width="50%", fig.cap ="GLM classification summary" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
We will now experiment with generative models like naive bayes classification. It assumes the features that go into the model is independent of each other.That is changing the value of one feature, does not directly influence or change the value of any of the other features used in the algorithm. When we analyzed the dataset it didn't appear to have independent features. An example of dependent feature would be age and education years, age and martial status etc. However, lets see how naive bayes performs with this assumption.
...
Naive bayes gives us an accuracy of 0.8009 with a lower prevalence than other algorithms but higher than plain guessing. However the confidence interval is lower and accuracy of the prediction is lower too. There are more false negatives as well. F1 score and AUC has increased. This indicates there are more true positives as well. However, given that this is a imbalanced dataset, we can try other advanced algorithms and compare this later.

```{r,echo=FALSE,out.width="50%", fig.cap ="naive bayes classification summary" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
knn raw model produces same accuracy of 0.8081, however, specificity is lower than sensitivity and prevalence is now 0.753. We now see the positive prediction is better. F1 score has improved. However, the AUC score is less than naive bayes. This indicates that the number of true positives are lesser than before. Balanced accuracy is better. Can we do better with accuracy and precision? 

```{r,echo=FALSE,out.width="50%", fig.cap ="knn classification summary" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
The accuracy of the knn tuned classification is 0.8098 with a sensitivity of 0.9032 and specificity of 0.5237. This is better than the previous models. Prevalence is just about the same as raw knn. Balanced accuracy has improved. F1 score is also at ~ 88% and AUC has improved a bit.

```{r,echo=FALSE,out.width="50%", fig.cap ="knn tune classification summary" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
The accuracy of recursive partitioning is 0.8211, sensitivity is higher at 0.91 and specificity is at 0.5487 with prevalence about the same. Balanced accuracy has improved. Confidence intervals for positive and negative has improved as well. The F1 score for rpart is 0.88467 and AUC is 0.7294 which is a positive sign.

```{r,echo=FALSE,out.width="50%", fig.cap ="rpart classification summary" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
The accuracy of random forest is at 0.8235, sensitivity is at 0.9164 and specificity is at 0.5390. Prevalence is about the same when compared to knn and rpart classification models. The confidence intervals is about the same as rpart. The F1 score is 0.88674 is the best so far. AUC also on the positive side

```{r,echo=FALSE,out.width="50%", fig.cap ="random forest classification summary" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
When we add "class" as one the feature (with some missing occupation data (5%)) we see that the accuracy is 0.8283 and sensitivity and specificity are 0.9086 and 0.5822 respectively with a F1 score of 0.88864.

Adding class has a negative impact on the accuracy and precision when compared to occupation. We can further investigate into how often a specific feature is used in the predictions using variable importance
...
# hoursperweek   844.8170

# maritalstatus  921.5902

# relationship  1162.1617

# occupation    1038.7262

# class          430.0374

```

We can see that age, years of education, and relationship are the most used and sex is the least used feature. We can also see that occupation did have a greater impact on decisioning even though we had 5% bad data. However, it does impact the precision of the prediction.
...
# Mcnemar's Test P-Value : 1.213e-08       
...
The accuracy of random forest with tuning is at 0.8228, sensitivity is at 0.9232 and specificity is at 0.5153. Prevalence is about the same when compared to knn and rpart classification models. The confidence intervals is just about the same as rpart and random forest without tuning. Interestingly enough, the accuracy is slightly lesser than the random forest test without tuning. Overall there was no further improvement in accuracy. The tuning resulted in over-fitting and didn't provide additional value to precision or accuracy.AUC also decreased slightly.

```{r,echo=FALSE,out.width="50%", fig.cap ="random forest tuned classification summary" , warning=FALSE, regfloat=FALSE}
library(ggplot2)  # For plotting
library(png) 
...
When we added the experimental occupation and class data into the dataset for rf tuned test we see that the accuracy improves to 0.8403 and sensitivity is 0.9223 and specificity is 0.5891 with prevalence being the same as before and balanced accuracy improved to 0.7557. The F1 score for this is 0.896
...
The accuracy of the predictions is backed by sensitivity of 0.9163 and specificity of 0.539. The F1 score 0.8867 vouches for the sensitivity and specificity. AUC is around 0.72 and is considered as acceptable considering some degree of imbalance in the dataset. 
...
```{r echo=TRUE,tidy=TRUE, warning=FALSE, tidy.opts=list(comment=TRUE), code=readLines('CensusPay.R')}
