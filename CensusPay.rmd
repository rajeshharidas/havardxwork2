...
#initialize the knitr package to echo by default, hide warnings and messages, show tidy display
knitr::opts_chunk$set(echo = TRUE,warning = FALSE,class.source="special-code", class.output="special-code", message = FALSE, tidy = TRUE, tidy.opts = list(comment = TRUE))
...
```{r echo=FALSE,include=FALSE,warning=FALSE,message=FALSE, cache=TRUE, code=readLines('DatasetProcessingCode.R')}
# cache the dataset loading code 
```
...
```{r warning=FALSE, regfloat=FALSE, class.source='special-code'}
library(tidyverse)
library(caret)
...
```{r,echo=FALSE,fig.retina = 2, fig.cap ="Income distribution" , warning=FALSE, regfloat=FALSE, fig.align='center'}
...
2) Remove all ? marks data from all the columns. Impute ? values to the modes in categorical columns like class, and occupation
3) Remove intermediate columns and columns that are not used in the project
4) Rename column names and column values to exclude non-alphanumeric characters
5) convert character labels to factors

```{r eval=FALSE,class.source='special-code'}
...
```{r,echo=FALSE,out.width="75%", fig.cap ="Comparison plots for numerical variables" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
img_path <- "images/comparevars1.png"
knitr::include_graphics(img_path)
```
Here are the comparison plots (generated by GGally package) for the numerical variables like age, eduyears and hoursperweek. There are very less people who have less than 4 years of education.Most people complete at least 9-10 years of education between 25-80 years age.Significant group of people stopped at elementary school. Significant group of people put in more than 40 hours per week past their retirement age. People who have 8-10 years of education put in more hours per week than others.

```{r,echo=FALSE,out.width="75%", fig.cap ="Comparison plots for categorical variables" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
img_path <- "images/comparevars2.png"
knitr::include_graphics(img_path)
```
...
```{r,echo=FALSE,out.width="50%", fig.cap ="Education statistics" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="75%", fig.cap ="Education income distribution" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
Interestingly enough, in comparison to Bachelors and some college, group with masters have lesser above-50K. Additionally, the group that has at least 12 years of education have more above-50K than others. On an average the participants have 10 years of education with a standard deviation of 2.4. 

```{r,echo=FALSE,out.width="75%", fig.cap ="Occupation distribution" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
There are more at-or-below 50K earners in the high-school grad and less than 10 years of education categories. 

```{r,echo=FALSE,out.width="50%", fig.cap ="Years of education" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="50%", fig.cap ="Occupation frequency" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="50%", fig.cap ="Sex feature characteristics" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
knitr::include_graphics(img_path)
```
There are 67% more males in the workforce than females. The distribution also indicates that the number of males at above-50K are more than the number of females. Additionally, there are more males at-or-below-50K than females as well. There are more white-Americans in the labor force in both the above-50K and at-or-below-50K categories.
```{r,echo=FALSE,out.width="52%", fig.cap ="Race distribution" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
img_path <- "images/incomebyrace.png"
knitr::include_graphics(img_path)
```
```{r,echo=FALSE,out.width="60%", fig.cap ="Race distribution" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
img_path <- "images/racebarch.png"
knitr::include_graphics(img_path)
```
Black-Americans and other race follow next. The proportion of above-50K earners are lesser in other races when compared to white-Americans.
```{r,echo=FALSE,out.width="70%", fig.cap ="Marital Status distribution" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="50%", fig.cap ="Marital Status frequency" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="50%", fig.cap ="Relationship distribution" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
The number of husbands who earn more than 50K and at-or-below-50K are also more than the number of wives. Unmarried individuals also are more in the at-or-below-50K category. The number of people from age 30-45 are the most frequent in the distribution and the same group have more above-50K incomes. There are 12 outliers in the age group above 80 who earn Above-50K. There are similar outliers in age-sex and age-race comparisons. However, the numbers are not significant enough to skew the distributions.

```{r,echo=FALSE,out.width="60%", fig.cap ="Age distribution" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="50%", fig.cap ="Hours worked" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
img_path <- "images/hoursperweek.png"
knitr::include_graphics(img_path)
```
As shown in the figures the occupation and class categorical columns have more than 1600 Unknown data. As these are categorical data, we will impute them using mode.
```{r,echo=FALSE,out.width="50%", fig.cap ="Class distribution" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
knitr::include_graphics(img_path)
```
However, as this is 6% of the data, adding these to the predictor columns could skew the predictions and introduce errors. We will exclude this from our list of predictors. We will experiment with these two features in our final model. We also removed capital gains and loss columns because we do not have good realistic data for both of these columns. The final list of predictors/features used to predict the outcome income level are age, education, years of education (a.ka. eduyears), maritalstatus, relationship,race, sex, and hoursperweek. As analyzed in the above section, there is sufficient degree of variability and correlation between these features and how they impact the income levels based on socio-economic and cultural factors.

After the analysis we continue to choose to predict Above50K as our goal, even though we know the data is skewed, as in real life this is often the case.
As this is an imbalanced dataset, we will also experiment with some sampling up with 10 fold cross validation in some of the models.There are risks associated with over and under-sampling. We could over fit/estimate or loose some critical information. For this reason, I have not applied it to all the algorithms.

## Process

The high-level process methodology used in this project is along the same lines as the CRISP-DM (Cross Industrial Standard Process for Data Mining) methodology. We got a business understanding of the problem we are trying to solve. We analyzed the raw data. Next we prepare the dataset from the raw data. 

```{r,echo=FALSE,out.width="65%", fig.cap ="Process Methodology" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
img_path <- "images/crispdm.png"
knitr::include_graphics(img_path)
```
...
The lm function applies this formula on adultpay dataset. Where \({y}\) is the outcome and \(\epsilon\) is the error. Sum of \(\beta_{1}x_{1}\) through \(\beta_{8}x_{8}\) is a linear combination of \(x_{1}\) through \(x_{8}\).
\small
$$
\operatorname{y} = \alpha + \beta_{1}(\operatorname{age}) + \beta_{2}(\operatorname{eduyears}) + \beta_{3}(\operatorname{sex}) + \beta_{4}(\operatorname{race}) + \beta_{5}(\operatorname{hoursperweek}) + \beta_{6}(\operatorname{maritalstatus}) + \beta_{7}(\operatorname{relationship}) + \beta_{8}(\operatorname{education}) + \epsilon
$$
\normalsize
...
```{r,echo=FALSE,out.width="75%", fig.cap ="LM classification summary" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
We will now experiment with the general linear model (glm). We will be consistent with the features used across all algorithms. Glm uses the following formula 
\tiny
$$
\log\left[ \frac { P( \operatorname{y} = \operatorname{1} ) }{ 1 - P( \operatorname{y} = \operatorname{1} ) } \right] = \alpha + \beta_{1}(\operatorname{age}) + \beta_{2}(\operatorname{eduyears}) + \beta_{3}(\operatorname{sex}) + \beta_{4}(\operatorname{race}) + \beta_{5}(\operatorname{hoursperweek}) + \beta_{6}(\operatorname{maritalstatus}) + \beta_{7}(\operatorname{relationship}) + \beta_{8}(\operatorname{education}))
$$
\normalsize

Where \({y}\) is the outcome and \(\epsilon\) is the error. Sum of \(\beta_{1}x_{1}\) through \(\beta_{8}x_{8}\) is a linear combination of \(x_{1}\) through \(x_{8}\)

Logistics model assures that the estimate \(\mathit{Pr (Y = 1 \mid X = x)}\) is between 0 and 1. Glm converts probability into log odds - \(\log\left[ \frac { P( \operatorname{y} = \operatorname{1} ) }{ 1 - P( \operatorname{y} = \operatorname{1} ) } \right]\). We will then translate the probability into a decision rule to predict Above50K if probability is > 0.5. We then compare the predictions to the outcomes using confusionMatrix.
...
```{r,echo=FALSE,out.width="65%", fig.cap ="GLM classification summary" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="75%", fig.cap ="naive bayes classification summary" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE, fig.retina = 2, fig.cap ="knn tuning" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="75%", fig.cap ="knn classification summary" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="75%", fig.cap ="knn tune classification summary" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE, out.width="75%", fig.cap ="rpart accuracy" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="65%", fig.cap ="rpart decision tree" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
img_path <- "images/rpartdectree.png"
knitr::include_graphics(img_path)
```
...
```{r,echo=FALSE,out.width="75%", fig.cap ="rpart classification summary" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="75%", fig.cap ="random forest classification summary" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```{r,echo=FALSE,out.width="65%", fig.cap ="random forest classification errors" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
Lets tune this model just like the KNN3 classification and see if we can do better. We will also use 10 x 10 repeated cross-validation to upscale sampling and compensate for an imbalanced dataset.
...
nodesize <- seq(1, 51, 10)
 ...
 train_rf_2 <- randomForest(y ~ age + eduyears + sex + race + hoursperweek + maritalstatus + relationship, data=temp,nodesize = nodesize[which.max(acc)],preProcess = c("scale", "center"), trControl = ctrl)
...
\begin{verbatim}
 Confusion Matrix and Statistics
  ...
  Prediction FALSE TRUE
      ...
      P-Value [Acc > NIR] : < 2.2e-16       
                    Kappa : 0.5441          
   Mcnemar's Test P-Value : 1.325e-07       
              Sensitivity : 0.9200      
              Specificity : 0.5947    
...
The accuracy of random forest with tuning is at 0.8221, sensitivity is at 0.916 and specificity is at 0.533. Prevalence is about the same when compared to knn and rpart classification models. The confidence intervals is just about the same as rpart and random forest without tuning.AUC improved slightly. In addition to the accuracy, precision increased to 0.8859.

```{r,echo=FALSE, out.width="65%", fig.cap ="rf tuning" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
img_path <- "images/rf2tune.png"
knitr::include_graphics(img_path)
```
When we added the experimental occupation and class data into the dataset for rf tuned test we see that the accuracy improves to 0.83996 and sensitivity is 0.9205 and specificity is 0.5947 with prevalence being the same as before and balanced accuracy improved to 0.7574. The F1 score for this is 0.896
```{r,echo=FALSE,out.width="65%", fig.cap ="random forest tuned classification summary" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```
The out-of-bag errors and errors for Above-50K and AtBelow50K classes are shown in the classification errors figure. You can see the errors reduce as the number of trees are added and then plateau after a while. The errors are around 16% and are within acceptable limits.
```{r,echo=FALSE,out.width="65%", fig.cap ="tuned random forest classification errors" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
...
```
We can see the variable importance with occupation and class.
```{r,echo=FALSE,out.width="60%", fig.cap ="tuned random forest variable importance" , warning=FALSE, regfloat=FALSE, fig.align='center'}
library(ggplot2)  # For plotting
library(png) 
img_path <- "images/rf2varimp.png"
knitr::include_graphics(img_path)
```

When we compare our final model with glm, naive bayes, knn and rpart the random forest model took quite a bit of time to generate. Hence the cost of generation is high. However, on the flip side the accuracy, CI, precision is higher than other models. Prevalence plateaus once the accuracy and precision are high. The logistic and generative models like GLM, and Naive Bayes could return a faster response but sacrifices accuracy and precision and has a decent prevalence. Naive Bayes achieves a better accuracy than glm and has compatible sensitivity and specificity and lower prevalence when compared to random forest and knn. KNN is a mid-range model with speed faster than random forest but with lower accuracy and precision. It is slower than Naive Bayes and Glm.

If my objective were to get the outcomes faster with slightly lesser accuracy then I would have gone with Naive Bayes. However, if you look closer at the classification models, there is a high degree of imbalance in the dataset and naive bayes doesn't do very well when the dataset is imbalanced. F1 score of random forest is higher than naive bayes. For this reason I chose random forest over naive bayes. Further more as this is a Census pay prediction application, the frequency of runs would be lesser compared to applications like detecting if a person will get a heart attack in the next 24 hours. In such a scenario getting a faster prediction at a lower accuracy can be used to err on the safe side.

# Results

This project created a machine learning model that predicts income level of adults based on 8 shortlisted attributes and 2 experimental attributes. This model was run on a test dataset that included an income level indicator, allowing us to compare the predicted and actual value.

We chose random forest as the final model after comparison. The model returned an accuracy of 82 to 83% while predicting annual income more than $50,000 annually. If occupation and class (with 5% unknown data) are included as features then accuracy changes to 83 to 84%. A summary table of results is given below:
...
This machine learning model was able to predict annual incomes of persons in US based (1994) on 8 parameters with an accuracy of 82 to 83%. This model can be applied to data from other census years as well. The model will perform better if the training set is updated with new data that is confirmed for correctness, that is, the label value is the real life value, and not the predicted value. Additional data for certain features like marital status, occupation, and class will make the model better. The model can continuously learn from changing data in the training set to adapt to new parameters, thus improving its accuracy and other metrics

Further improvements could be made by using additional hybrid random oversampling and/or under-sampling techniques like SMOTE or ROSE to upscale the imbalanced datasets.
...
Additionally, we could also use advanced algorithms like Ada Boost Gradient Boost Trees, support Vector Machines, Neural Networks, and Deep Learning to improve the predicted outcomes.

\newpage

### Appendix A - Complete code

```{r eval=FALSE,tidy=TRUE, warning=FALSE,message=FALSE, tidy.opts=list(comment=TRUE), cache=TRUE, class.source='special-code', code=readLines('CensusPay.R')}

```
### Appendix B - Code Execution

The code in this project takes long time to execute. Please find the execution summary at the link below.

\url{https://github.com/rajeshharidas/havardxwork2/blob/main/CensusPayExecutionSummary.pdf}

\url{https://github.com/rajeshharidas/havardxwork2/blob/main/DatasetProcessingCode.pdf}

\url{https://github.com/rajeshharidas/havardxwork2/blob/main/CensusPayExecutionSummary.html}

\url{https://github.com/rajeshharidas/havardxwork2/blob/main/DatasetProcessingCode.html}
...
ML-Friendly kaggle dataset for adult census income - \url{https://www.kaggle.com/uciml/adult-census-income}