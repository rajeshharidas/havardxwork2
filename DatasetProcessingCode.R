...
dl <- tempfile()
download.file("https://github.com/rajeshharidas/havardxwork2/raw/main/adult.csv.zip",dl)

#read all the data into R dataset
adultpay <- fread(text = gsub(",", "\t", readLines(unzip(dl, "adult.csv"))),
...
adultpayclean <- adultpay %>% filter (native.country == 'United-States') %>% 
                 ...
                 rename(c(eduyears=education.num,maritalstatus=marital.status,hoursperweek=hours.per.week,native=native.country)) %>%
                 mutate (maritalstatus = ifelse(maritalstatus == '?','Unknown',str_replace_all(maritalstatus,"-",""))) %>% 
                 mutate (occupation = ifelse(occupation == '?','Unknown',str_replace_all(occupation,"-",""))) %>% 
                 mutate (education = ifelse(education == '?','Unknown',str_replace_all(education,"-",""))) %>%
                 mutate (relationship = ifelse(relationship == '?','Unknown',str_replace_all(relationship,"-",""))) %>%
                 mutate (native = ifelse(native == '?','Unknown',str_replace_all(native,"-",""))) %>%
                 mutate (income = ifelse(income == '?','Unknown',str_replace_all(income,"<=50K","AtBelow50K"))) %>%
                 mutate (income = ifelse(income == '?','Unknown',str_replace_all(income,">50K","Above50K")))
  
# R 4.0 or later:
#convert all the character labels to factors
adultpayclean <- as.data.frame(adultpayclean) %>% mutate(education = as.factor(education),
...
test_index <- createDataPartition(y = adultpayclean$income, times = 1, p = 0.1, list = FALSE)
